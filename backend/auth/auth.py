...
AUTH0_DOMAIN = 'holecska.eu.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'capstone_identifier'
...
def get_token_auth_header():
    if 'Authorization' not in request.headers:
        raise AuthError({
            'code': 'invalid_header',
            'description': ('Authorization is missing in the JWT Token\'s '
                            'Header')
        }, 401)
    ...
    if len(header_parts) != 2:
        raise AuthError({
            ...
            'description': 'Unauthorized Person. - Token has not two parts'
        }, 401)
    elif header_parts[0].lower() != 'bearer':
        raise AuthError({
            'code': 'Unauthorized - invalid Token',
            'description': 'Unauthorized Person. - Not bearer Token'
        }, 401)
...
def check_permission(permission, payload):
    ...
    if permission not in payload['permissions']:
        raise AuthError({
            'code': 'forbidden',
            'description': 'Permission not found. - method not allowed'
        }, 401)
